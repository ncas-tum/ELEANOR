...
def test(model_name, variability, fix, seed, params):
...
def train(model_name, variability, fix, seed, params, trial=None, writer=None):
    key = jrandom.key(seed)
    ...
    if fix:
        ...
        def update_lr(updates, state, value):
            return state
    else:

        def update_lr(updates, state, value):
            return transform.update(updates=updates, state=state, value=value)[1]
    ...
    for epoch in pbar:
        ...
        if writer is not None:
            ...
            logdir = f"review/{model_name}/{params['lr']}"
            if not os.path.exists(logdir):
                os.makedirs(logdir)

            df = pd.DataFrame(
                {
                    "Loss train": total_train_loss,
                    "Loss": total_test_loss,
                    "Epoch": jnp.arange(epoch + 1),
                    "Seed": [seed] * (epoch + 1),
                    "Learning rate": [params["lr"]] * (epoch + 1),
                    "Variability": [variability] * (epoch + 1),
                    "Model": [model_name] * (epoch + 1),
                }
            )
...
def _objective(model_name, variability, fix, seed, trial):
    params = {
        "lr": trial.suggest_float("lr", 1e-5, 1e-1, log=True),
        "optimizer": trial.suggest_categorical("optimizer", ["sgd", "adamax"]),
    }

    if model_name == "FeLIF" or model_name == "Bruno":
    ...
    print(params)
    return train(model_name, variability, fix, seed, params, trial)
...
if __name__ == "__main__":
    ...
    if args.fix:
        study_name = (
            ...
            if args.model == "FeLIF" or args.model == "Bruno"
            else f"{args.model}"
        )
    else:
        study_name = (
    ...
    if args.hpo:
        objective = partial(
            _objective, args.model, args.variability, args.fix, args.seed
        )
        ...
        test(args.model, args.variability, args.fix, args.seed, vars(args))
    else:
        # writer = SummaryWriter(log_dir=f"bruno/{study_name}")
        writer = None
        train(
            args.model, args.variability, args.fix, args.seed, vars(args), writer=writer
        )
