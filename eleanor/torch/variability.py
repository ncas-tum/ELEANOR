...
def update_d2d_variability(model: torch.nn.Module, shape: Sequence[int]) -> None:
    ...
    Parameters
    ==========
    model: PyTrtorch.nn.Moduleee
        Model with the parameters that want to update the D2D.
    shape: Sequence[int]
        Shape of the variability array.
    """
...
def update_d2d_variability_name(
    ...
    """
    Update the device to device variability of all
    the D2DVar parameters with the same name..
...
def set_d2d_variability(
    ...
    if not isinstance(variability, Sequence):
        variability = [variability] * len(modules)

    assert len(variability) == len(modules), (
...
class D2DVar(torch.nn.Module):
    ...
    Example
    -------
    >>> param_var = D2DVar("param", 0.1)
    >>> param_with_variability = param_var(param, (10,))
    """
