...
class FeLIF(SpikingNeuron):
    def __init__(
        ...
        self._register_buffer("P_s", torch.tensor(P_s), learn_P_s)
        self._register_buffer("alpha", torch.tensor(alpha), learn_alpha)
        self._register_buffer("beta", torch.tensor(beta), learn_beta)
        self._register_buffer("gamma", np.exp(-dt / tau_m), learn_gamma)
        self._register_buffer("tau_p", torch.tensor(tau_p), False)
        self._register_buffer("dt", torch.tensor(dt), False)
    ...
    def reset_mem(self):
        ...
        self.pol = torch.full_like(self.pol, -self.P_s, device=self.pol.device)
