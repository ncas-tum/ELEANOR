...
class FeLIFV2(StatefulLayer):
    ...
    def __call__(
        ...
        def step(state, synaptic_input):
            ...
            I_p_new = (jnp.sign(E) * self.P_s - p) * self.A / tau
            dp = I_p_new / self.A
            p_new = jnp.clip(p + 1e-3 * self.dt * dp, -self.P_s, self.P_s)

            I_leak = (
                self.I_0 * self.A * jnp.expm1(v / self.V_t) + self.I_dsc
            ) * jnp.sign(v)
            dv = (synaptic_input - I_leak - I_p_new) / self.C_tot
        ...
        def step2(v, p, synaptic_input):
            E = v * self.cap_divider - p * self.depol_divider

            tau = tau_fn(E)

            I_p_new = (jnp.sign(E) * self.P_s - p) * self.A / jax.lax.stop_gradient(tau)
            dp = I_p_new / self.A
        ...
        (v_inner, p_inner, _), _ = jax.lax.scan(
            step,
            (v, p, jnp.zeros_like(p)),
            jnp.repeat(synaptic_input[None, ...], 1000, axis=0),
        )
        v_outer, p_outer = step2(v, p, synaptic_input)
