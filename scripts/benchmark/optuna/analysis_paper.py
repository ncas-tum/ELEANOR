...
from eleanor.models.jax.variability import StaticWrapper, update_d2d_variability
...
def _objective(model_name, quant_bits, use_bias, variability, update_epochs, trial, seed):
    ...
    for curr_epoch in pbar:
        ...
        if update_epochs>0 and (curr_epoch+1)%update_epochs:
            key, subkey = jax.random.split(key)
            model = update_d2d_variability(model, subkey)

    logdir = f"export_paper/{quant_bits}bit_{model_name}/{variability}"
...
print(args.model, args.quantization, args.variability, args.seed)
...
objective = partial(_objective, args.model, args.quantization, False, args.variability, args.update)
...
logdir = f"results_paper/{args.quantization}bit_{args.model}/{args.variability}"
if not os.path.exists(logdir):
    os.makedirs(logdir)
